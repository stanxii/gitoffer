...
// Fetches a page of jobs for a particular title/company/salary/....
// Relies on the custom API middleware defined in ../middleware/api.js.
function fetchJobs(queryString, nextPageUrl) {
  return {
    queryString,
    [CALL_API]: {
      types: [ JOBS_REQUEST, JOBS_SUCCESS, JOBS_FAILURE ],
      endpoint: `jobs/onepage/0/20`,
...
// Bails out if page is cached and user didn’t specifically request next page.
// Relies on Redux Thunk middleware.
export function loadJobs(queryString, nextPage) {
  return (dispatch, getState) => {
     const {
      nextPageUrl = `jobs/${queryString}/list`,
      pageCount = 0
    } =  {}

    return dispatch(fetchJobs(queryString, nextPageUrl))
  }
}
